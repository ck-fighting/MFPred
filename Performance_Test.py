...
from Data_Create import data_process_hanming1
import pandas as pd
import os
import numpy as np
...
PATH_Model_word2vec = 'Trained_model/NCY/GRU/word2vec_fold3'
PATH_Model_NCP = 'Trained_model/NCY/GRU/NCP_fold3'
PATH_Model_kmer = 'Trained_model/NCY/GRU/kmer_fold3'
PATH_Model_GCN = 'Trained_model/NCY/GRU/GCN_fold3'
PATH_Model_CNN = 'Trained_model/NCY/CNN/ResNet_SE_fold3'
...
Test_Data_word2vec, Test_Data_DCN, Test_Data_kmer, Test_Data_GCN, Test_Label = data_process_hanming1.test_data()  # 获取RNA标签信息
lstm_word2vec = torch.load(PATH_Model_word2vec)  # 调用模型BI-GRU+AM+Densenet
lstm_DCN = torch.load(PATH_Model_NCP)
...
data_loader_test_DCN = DataLoader(test_data_DCN, batch_size=32, shuffle=False,collate_fn=collate_fn)  # 用于将test_data数据划分批次，每个批次包含32个RNA序列，划分依据collate_fn函数
...
data_loader_test_GCN = DataLoader(test_data_GCN, batch_size=32, shuffle=False,collate_fn= collate_fn)
...
with torch.no_grad():  # 以下是为了保存模型
    ...
    for item_test_word2vec, item_test_DCN, item_test_kmer, item_test_GCN in zip(data_loader_test_word2vec, data_loader_test_DCN,
        ...
        merged_feature = torch.cat(( output_word2vec, output_DCN, output_GCN, output_kmer), dim=1)
...
Pre_Data.to_csv('Pre_Data_new/NCY/fold3.csv', encoding='gbk')
